...
public class HealthApiTest {
    ...
    public void healthTest() {
        ...
        assertThat(healthJson.read("$.components.palTracker.status", String.class)).isEqualTo("UP");

        this.restTemplate.postForEntity("/failure/set", null, String.class);

        response = this.restTemplate.getForEntity("/actuator/health", String.class);

        healthJson = parse(response.getBody());

        assertThat(healthJson.read("$.components.palTracker.status", String.class)).isEqualTo("DOWN");
    }
}
